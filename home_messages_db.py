...
def check_filepaths(user_input_files, toolname):
        ...
        if len(user_input_files) == 1:
            ...
            files = glob.glob(filename) + glob.glob(f"data/{toolname}/{filename}") # now fetch all files from this directory or the data/toolname directory
...
class HomeMessagesDB:
    ...
    def query_electricity(self):
        ...
        click.echo(elec_inp)
        if (elec_inp.lower().strip() == "import"):
        ...
        elif (elec_inp.lower().strip() == "export"):
